...
amended:
  ...
  - date: 2026-06-05
    note: "Wave-2 canary wiring (resolves the deferred invokes-canary seam; CF-4 / the wave-2 health-check item). canary is now authored (graph/canary/, an autonomous post-deploy live-health agent → HEALTHY/DEGRADED/BROKEN verdict), so wired the inbound invoke on land's side (caller-side convention, as for ship/deploy; canary declares no inbound edge). Frontmatter: added invokes canary. Body: replaced the 'Canary (wave 2) — not authored in wave 1' paragraph with a new 'Step 4 — Canary (post-deploy live health, input-gated)' sub-arc step describing canary as the wired third delivery step that feeds the live-confirmed gate, BUT input-gated on live prod traffic — built dormant, invoked only after a real deployment with traffic (prod-direct / prod-staging-first); for staging-only or a pre-launch product with no prod traffic, canary stays dormant and deploy's inline smoke check remains the live-confirmed signal. Exit gate-surface + the F7 invokes-canary seam updated to match (canary's verdict carried alongside smoke_health when active; 'wired' not 'wave 2'). No overstatement — canary runs only when there is real traffic to watch and never fabricates a baseline. Status bumped v0.2.0 → v0.3.0. Surgical edge+prose amend, not a re-render; gate/sub-arc/revert mechanics otherwise unchanged. NOTE: CF-4's wave-1 resolution (deploy's inline smoke check as the live-confirmed signal) is preserved — it is now explicitly the fallback when canary is dormant."
sources_lifted: 2
