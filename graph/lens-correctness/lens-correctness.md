...
description: Autonomously hunt logic and behavioural correctness defects in a target and return structured findings.
when-to-use: A review/design/plan stage's dispatch fans out a correctness pass over a diff or a design/plan doc.
...
# analytics — the loop
goals:
  - outcome: Real correctness defects (logic, edge-case, state, swallowed-error, boundary, race) are caught before they reach the build/land stages.
    metric: true-positive findings per review that survive merge-triage (confidence ≥ 75, not dropped by the validator).
    earns-keep: sustains a non-trivial survived-finding rate AND a measurable drop in escaped correctness defects vs the pre-lens baseline over N sprints.
  - outcome: The change lands with its correctness defects already surfaced, not discovered in production.
    metric: escaped-correctness-defect rate (defects traced to a reviewed change the lens did not flag) vs the transcript baseline.
    earns-keep: escaped-defect rate below baseline; if the lens never lowers it, it is cut or merged.
  - outcome: Findings are trustworthy enough to action, not noise the operator must wade through.
