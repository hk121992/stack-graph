...
amended:
  ...
  - date: 2026-06-05
    note: "Wave-2 invocation wiring — the three sub-path nodes named in the old process-seams are now authored, so wired their inbound invokes (caller-side convention, as for explore). Added to frontmatter: invokes debug, invokes design-implement, invokes optimise (each target declares no inbound edge by design). Body: replaced the deferred '→ debug (wave 2, not yet authored)' seam with a new 'Invoked sub-paths (within the span)' section describing WHEN build invokes each — debug = a unit fails acceptance_check and the cause is not diagnosable quickly in-span (root-cause-first, not guess-patch); design-implement = a UI implementation unit (production UI from an approved design); optimise = a perf-critical unit with a measurable objective and a correct baseline (generate-measure-select). Nothing still calls these deferred. Process-seams section retains only the genuinely-deferred wiring-pass edges (plan/review/reconcile loops, .worktreeinclude). Frontmatter comment block updated to mark debug/design-implement/optimise as wired caller-side. Node bumped v0.2.0 → v0.3.0. Surgical edge+prose amend, not a re-render; span mechanics unchanged."
sources_lifted: 5
