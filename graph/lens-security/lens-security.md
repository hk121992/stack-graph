...
primitive: agent
title: Security lens
description: Autonomously hunt exploitable security vulnerabilities in a target and return structured findings.
when-to-use: A review/design/plan stage's dispatch fans out a security pass over a diff or a design/plan doc.
...
# analytics — the loop
goals:
  - outcome: Exploitable vulnerabilities (injection, authz bypass, secret exposure, SSRF, unsafe deserialization, crypto misuse, trust-boundary breaks) are caught before the change lands.
    metric: true-positive security findings per review that survive merge-triage (not dropped by dedup, the confidence-gate, or the validator).
    earns-keep: sustains a non-trivial survived-finding rate AND a measurable drop in escaped vulnerabilities vs the pre-lens baseline over N sprints.
  - outcome: The change lands with its exploitable vulnerabilities already surfaced, not discovered after exploitation in production.
    metric: escaped-vuln rate (vulnerabilities traced to a reviewed change the lens did not flag) vs the transcript baseline.
    earns-keep: escaped-vuln rate below baseline; if the lens never lowers it, it is cut or merged.
  - outcome: Security findings are trustworthy enough to action, not theatre the operator must wade through — even under the deliberately lower reporting gate.
    metric: false-positive / suppression rate at merge-triage (findings dropped by dedup, confidence-gate, or validator).
    earns-keep: precision high enough that the lens's findings stay in the actionable tier rather than being routinely demoted to advisory.
