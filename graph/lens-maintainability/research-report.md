...
researcher_adequacy_note: |
  ...
  a kind: reference artefact in graph/_refs/ that the dispatching stage passes into the
...
`lens-dispatch` reference the dispatching stage follows). It explicitly does **not** own correctness
...
**one parameterised lens** with a `target` mode in the body (D34 — one node, one primitive;
`target` is a body branch of the single lens node, never a separate node). The CE
