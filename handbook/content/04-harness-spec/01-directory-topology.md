...
| `renderer` | the vendored workspace render (0.5.0+): the bound surface roots it is pointed at (`handbook-root`, `dashboard-root`, `canvas-root`, `brand-root`, `graph-root`, and optional `graph-local` — the harness's local graph-overlay manifest composed onto the vendored graph so the surface shows the **whole** graph by owner), the output/portal path, and degraded-mode policy. The renderer ships in the plugin; this binding overlays it onto the harness's surfaces |
...
renderer:                              # the plugin's vendored workspace render, overlaid onto these surfaces
  handbook-root: workspace/handbook/content
  dashboard-root: workspace/product
  canvas-root: workspace/product       # holds canvas.json (regenerated by strategy-curator refresh-canvas)
  brand-root: workspace/brand
  graph-root: stack-graph-plugin/workspace/graph   # the vendored (sg) graph data
  graph-local: workspace/graph/local.json          # optional: the harness's local graph overlay (composed by owner)
